...
export default function Notification() {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {/* Dropdown Notifikasi */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-xl border border-gray-100 py-3 z-[1050] overflow-hidden text-gray-800 animate-in fade-in zoom-in-95 duration-150">
          <div className="flex items-center justify-between px-4 pb-2 border-b border-gray-100">
            <h3 className="font-bold text-sm text-[#0d3b2e] flex items-center gap-2">
              Notifikasi
              {unreadCount > 0 && (
            ...
            {unreadCount > 0 && (
              <button
                ...
                disabled={isLoading}
                className="text-xs text-[#3dbd84] hover:text-[#1a5c44] font-medium transition-colors cursor-pointer"
              >
                Tandai semua dibaca
              </button>
            )}
