  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 space-y-4">
      ...
      {!verified ? (
                ...
                <ul className="divide-y divide-gray-100">
                  {filteredWarga.map((warga) => (
                    <li key={warga.id}>
                      ...
                      >
                        <div>
                          <p className="font-bold text-sm text-[#0d3b2e]">{warga.nama}</p>
                          <p className="text-xs text-gray-500">{warga.peran} {warga.keterangan_peran ? `(${warga.keterangan_peran})` : ""}</p>
                        </div>
                        <span className="text-xs font-mono font-bold text-[#1a5c44] bg-[#3dbd84]/10 px-2 py-1 rounded">
                          {warga.id}
                        </span>
                      </button>
                    </li>
        ...
        selectedWarga && (
          <div className="p-4 bg-[#3dbd84]/10 border border-[#3dbd84]/30 rounded-xl flex items-start gap-4 animate-in fade-in duration-300">
            <div className="w-10 h-10 rounded-full bg-[#1a5c44] text-white flex items-center justify-center font-bold text-lg flex-shrink-0">
              {selectedWarga.nama.charAt(0)}
            </div>
            <div>
              <p className="font-bold text-[#0d3b2e]">{selectedWarga.nama}</p>
              <p className="text-xs text-[#1a5c44] font-mono mt-0.5 font-bold">
                ID: {selectedWarga.id}
              </p>
            ...
            <div className="ml-auto flex items-center justify-center h-full">
               <span className="bg-[#3dbd84] text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                  Terverifikasi
               </span>
            </div>
          </div>
